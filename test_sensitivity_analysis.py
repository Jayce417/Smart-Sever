#!/usr/bin/env python3
"""
Test script for the sensitivity-analysis API endpoint with image display in dialog
"""
...
def test_sensitivity_analysis():
    ...
    # API endpoint
    url = "http://localhost:8000/api/sensitivity-analysis"
    ...
    try:
        ...
        print(f"URL: {url}")
        print(f"Variable: {test_data['varname']}")
        print()
        ...
        if response.status_code == 200:
            ...
            print(f"Node Type: {result.get('node_type', 'N/A')}")
...
if __name__ == "__main__":
    print("=== Sensitivity Analysis API Test ===\n")
    ...
    def payoff_fn(values, probabilities, branches):
        print(f"{values=},{probabilities=},{branches=}")
        bid = values["bid"] if "bid" in values.keys() else 0
        competitor_bid = (
            values["competitor_bid"] 
            if "competitor_bid" in values.keys() else 0
...
def payoff_fn_ev(values,probabilities,branches):
...
def payoff_fn_utility(values,probabilities,branches):
    import math
    R = 1000
    value_keys = values.keys()
    prob_keys = probabilities.keys()
...
def payoff_fn_certainty(values,probabilities,branches):
    import math
    R = 1000
    value_keys = values.keys()
    prob_keys = probabilities.keys()
